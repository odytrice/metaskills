...
- Only `issue-refine` moves an item to ready, updates a body, or splits (parent kept, native sub-issues). This skill writes nothing but `.tmp-*` body files and an optional ledger under `Docs/plans/`.
...
5. **Track** item, wave, state (`needs-answers` / `clarified` / `refined->ready` / `skipped` / `deferred`), notes; in chat, or `Docs/plans/YYYY-MM-DD-backlog-refine.md` for a large backlog.
