...
Bash-only (Linux and Linux containers); Windows and PowerShell coverage
removed. Fixtures use resolved temp paths because installers deliberately
...
class InstallerCases:
    def setUp(self):
        ...
        write(
            self.repo / "skills/demo/SKILL.md",
            "---\nname: demo\ndescription: Fixture skill\n---\nBody\n",
        )
        write(self.repo / "commands/demo.md", "Use demo with $ARGUMENTS\n")
        for dialect in ("claude", "opencode"):
            write(
                self.repo / f"agents/{dialect}/worker.md",
                "---\nname: worker\n---\nBody\n",
            )
        write(
            self.repo / "agents/codex/worker.toml",
    ...
    def assert_rejected_read_only(self):
        ...
        outside = (
            snapshot(self.base / "outside")
            if (self.base / "outside").is_dir()
            else None
        )
        for mode in ("", "dry"):
    ...
    def test_clean_install_reinstall(self):
        ...
        self.assertEqual(
            (self.home / ROOTS[0] / "demo/SKILL.md").read_bytes(),
